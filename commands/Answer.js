...
zokou({nomCom:"ian",reaction:"📡",categorie:"IA"},async(dest,zk,commandeOptions)=>{
  ...
  const {repondre,ms,arg}=commandeOptions;
    ...
    if(!arg || !arg[0])
    {return repondre("HI IF YOU ARE A GIRL DON TECH INC LOVES YOU⭕😘❤️❤️.")}
    //var quest = arg.join(' ');
  try{
...
.catch(error => {
  ...
  zokou({ nomCom: "dalle", reaction: "📡", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*powered by FI BOT-MD*';
...
zokou({ nomCom: "fi bot", reaction: "🤔", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    const { repondre, arg, ms } = commandeOptions;
